...
let movie_id = '';
const like_btn = document.getElementById("like_movieBtn2");
const url = "https://yts-proxy.now.sh/list_movies.json?sort_by=year";
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      like.map((item, index) => {
        ...
        document.querySelector("#like_movieBtn2").append(like_movie_btn);
      });
    });
}
